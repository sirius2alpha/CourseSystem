...
谢怡雯：在本次实验中，我主要负责了后端的用户登录功能和学生一系列的操作。第一次写vue-springboot-mysql框架的网站，开始有点手足无措。于是多方查找资料，以及看视频等等，在大概理解的基础上开始着手，选择用MVC的设计模式编写。以我们系统中的用户User对象为例，我们需要分别编写UserMapper，UserService，和 UserController 来管理与 User 相关的操作。其中 User 对象类充当了模型（Model）的角色，用于处理应用程序的数据和业 务逻辑。UserMapper 充当了数据访问层（Data Access Layer）的角色，负责与数据库进行交互，将数据存储和检索操作封装在数据访问对象（Data Access Object，DAO）中。UserService 充当了业务逻辑层（Business Logic Layer）的角色，负责处理业务逻辑，将业务操作封装在服务（Service）中。最后，UserController 充 当了控制器（Controller）的角色，负责处理用户请求，并将请求发送给对应的模型进行处理。 在 MVC 架构中，模型、视图和控制器各自独立，彼此之间耦合度较低。在实际撰写过程也遇到了许多问题，比如开始返回的result的java类中的数据是一个数组套数组，但是前端无法解析数组内容，而需要返回的是json格式。于是反复查资料测试后，首先依赖json库，运用writeasstring函数将返回的数组转化为json格式的string。在本次实验中，我学到了很多，是第一次独立完成的一个网站后端，在不断的写代码改bug中收获了很多，也是对我未来的开发道路的启蒙。



葛芸菲：这次的项目是我第一次和团队成员一起完成前后端分离的项目，小伙伴们各有所长，这四周的合作让我收获良多，感谢小伙伴们的支持与鼓励！

从一整个项目流程中，我们获得了许多宝贵的经验和教训——

1. **项目管理的重要性**：通过明确的时间线、任务分配和里程碑，我们确保项目按计划进行，第一周前端框架，第二周接口文档，第三周前后端齐头并进，第四周完善后端，我理解了团队协作和有效沟通的重要性。
2. **用户体验的中心性**：我们认识到不仅要满足功能要求，还要确保用户界面友好、直观。通过与真实用户（让其他小伙伴测试）的交互，我们收到了反馈，不断地优化和调整，确保用户体验满意。
3. **技术的应用与挑战**：在开发过程中，我们遇到了许多技术上的挑战，从Vue到Mybatis到Springboot，这些经验使我们更加适应快速变化的技术环境。
4. **持续测试的必要性**：我们认识到，在开发过程中持续进行测试是非常重要的，这不仅帮助我们发现问题，还确保了系统的稳定性和可靠性。我们学会了使用ApiFox来管理和测试接口，更好地完成前后端分离项目。
5. **需求变更的应对**：在项目进行中，需求可能会发生变更，比如教师的需求就更改过两次。我们学会了如何灵活应对这些变更，确保项目能够顺利进行。
6. **跨领域的合作**：这个项目涉及到多个领域的知识，我们学会了如何与各有所长的小伙伴精诚合作，集各方的知识和经验，共同推进项目。
7. **反思与改进**：每当项目的某个阶段完成后，我们都会进行回顾和反思，总结经验教训，不断优化工作流程和方法。我们从前后端的交互中明白了，开发初期就要确定所有需求，并定好前后端路径与接口，否则后续会出现很多问题。
8. **实际应用与理论知识的结合**：通过这个项目，我们将在学习中得到的理论知识应用于实践，从一开始看视频快速入门，到在项目中绞尽脑汁把所学知识应用起来，这加深了我们对知识的理解并增强了应用能力。
9. **问题解决能力的提高**：面对各种预料之外的问题和挑战，我们学会了迅速定位问题、分析原因和采取有效的解决策略。
10. **成长与自我提升**：通过这一系列的经验和挑战，我们每个团队成员都在各自的领域中得到了成长和进步。

总之，这个项目不仅带给我们宝贵的实践经验，更重要的是，它锻炼了我们的团队协作、问题解决和项目管理能力，为我们在未来的职业生涯中打下了坚实的基础。
